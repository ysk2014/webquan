define([
	...
	return React.createClass({
		...
		render: function() {
			...
			if(nav==0) {
				if(_this.state.cloumns['square']) {
					if(_this.state.cloumns['square']['view']) {
						console.log(_this.state.cloumns['square']['view']);
						var cloumns = _this.state.cloumns['square']['view'];
					} else if(_this.state.cloumns['square']['update_time']) {
						var cloumns = _this.state.cloumns['square']['update_time'];
					} else if(_this.state.cloumns['square']['care']) {
						var cloumns = _this.state.cloumns['square']['care'];
					} else if(_this.state.cloumns['square']['addtime']) {
						var cloumns = _this.state.cloumns['square']['addtime'];
					}
				} else {
					var cloumns = [];
				}
				var cloumns = _this.state.cloumns['square'] ? _this.state.cloumns['square'] : [];
			} else if(nav==1) {
