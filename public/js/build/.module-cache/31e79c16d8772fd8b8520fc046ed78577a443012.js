define([
	...
	return React.createClass({
		...
		render: function() {
			...
			if(nav==0) {
				var articles = _this.state.cloumns['square'] ? _this.state.cloumns['square'] : [];
				var articles = _this.state.cloumns['square'] ? _this.state.cloumns['square'].map(function(d,i) {
					return (
				...
				var articles = _this.state.cloumns['care'] ? _this.state.cloumns['care'] : [];
			} else {
				var articles = _this.state.cloumns['me'] ? _this.state.cloumns['me'] : [];
				var articles = _this.state.cloumns['me'] ? _this.state.cloumns['me'].map(function(d,i) {
					return (
						React.createElement("li", {key: d.id, className: "cloumn"}, 
							React.createElement("a", {href: "/cloumn/"+d.id}, 
								React.createElement("div", {className: "logo", style: {background:'url("/image/cloumn.jpg") no-repeat center',backgroundSize:'cover'}}), 
