	...
	var mixin = {
		...
		handleOver: function(event) {
			if($(event.target).find('span:eq(0)').html() =='正在') {
				$(event.target).find('span:eq(0)').html('取消关注');
			}
		},
		handleOut: function(event) {
			if($(event.target).find('span:eq(0)').html() =='取消关注') {
				$(event.target).find('span:eq(0)').html('正在关注');
			}
		},
	}
					...
					React.createElement("li", {key: d.id, "data-cid": d.id, "data-key": i, className: "cloumn"}, 
						React.createElement("a", {href: "/cloumn/"+d.id}, 
							...
							React.createElement("p", null, 
								...
								React.createElement("a", {className: "btn-success", "data-care": d.myCare ? d.myCare : false, onClick: _this.handleCare, onMouseOver: _this.handleOver, onMouseOut: _this.handleOut}, d.myCare ? '正在' : '添加', "关注")
							)
						)
					)
				);
