				...
				if(success) {
					if(!data.error) {
						...
						if(_this.state.cloumns[way]) {
							...
							if(way=='view') {
								_this.state.cloumns = {'view':data.data};
								_this.state.next = {'view':data.next};
								_this.state.page = {'view':parseInt(page)+1};
							} else if(way=='update_time') {
								console.log(111);
								_this.state.cloumns = {'update_time':data.data};
								_this.state.next = {'update_time':data.next};
								_this.state.page = {'update_time':parseInt(page)+1};
							} else if(way=='care') {
								_this.state.cloumns = {'care':data.data};
								_this.state.next = {'care':data.next};
								_this.state.page = {'care':parseInt(page)+1};
							} else if(way=='addtime') {
								_this.state.cloumns = {'addtime':data.data};
								_this.state.next = {'addtime':data.next};
								_this.state.page = {'addtime':parseInt(page)+1};
							}
						}
						_this.setState({
		...
		handleMore: function() {
			...
			if(_this.state.nav==0) {
				if(_this.state.navChild==0) {
					...
					var page = _this.state.page['update_time'] ? _this.state.page['update_time'] : o
				}
			}
		},
		// 一级导航切换
