		...
		render: function() {
			...
			if(this.state.cloumns.length>0) {
				var cloumns = _this.state.cloumns.map(function(d,i) {
					return (React.createElement("option", {key: d.id, "data-cid": d.id, className: (_this.state.selected===d.id) ? "tag selected" : "tag", onClick: this.handleSelect.bind(this,i)}, d.name));
				}.bind(this));
			} else {
				var cloumns = null;
			}
