				...
				CloumnModel.delCare(dataObj,function(success,data) {
					if(success) {
						if(!data.error) {
							if(nav==0) {
								if(navChild==0) {
									_this.state.cloumns['view'][index]['myCare'] = false;
								} else if(navChild==1) {
									_this.state.cloumns['update_time'][index]['myCare'] = false;
								} else if(navChild==2) {
									_this.state.cloumns['care'][index]['myCare'] = false;
								} else if(navChild==3) {
									_this.state.cloumns['addtime'][index]['myCare'] = false;
								}
							} else if(nav==1) {
								_this.state.cloumns['myCare'][index]['myCare'] = false;
							} else if(nav==2) {
								_this.state.cloumns['me'][index]['myCare'] = false;
							}
						} else {
							Tooltip(data.msg);
						}
