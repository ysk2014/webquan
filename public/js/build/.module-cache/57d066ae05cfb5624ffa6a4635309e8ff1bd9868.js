define([
	...
	return React.createClass({
		...
		dealTags: function(event) {
			if(event.which==32 && WQ.trim(event.target.value)!='') {
				$(event.target).before('<span class="tag selected">'+event.target.value+'</span>');
				$(event.target).val('');
			} else if(event.which==8 && event.target.value=='') {
