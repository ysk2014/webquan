	...
	return React.createClass({
		...
		dealTags: function(event) {
			...
			if((event.which==13 || event.which==32) && WQ.trim(event.target.value)!='') {
				...
				if(_this.state.tags.length>2 || WQ.inArray(_this.state.inputTag,_this.state.tags)) {
					ele.val('');
					return;
				}
		...
		dialogShow: function(event) {
			...
			var selectTag = function(name) {
				ele.parents('ul').siblings('input[type=text]').before('<span class="tag selected">'+WQ.trim(name)+'<span class="remove">x</span></span>');
				_this.state.tags.push(WQ.trim(name));
				_this.setState({
					tags: _this.state.tags,
					inputTag: ''
				});
				input.siblings('.selected').find('.remove').on('click',function() {
					var index = $(this).parent().index();
					_this.state.tags.splice(index,1);
					$(this).parent().remove();
				});
			...
			Dialog({
				...
				ok: function(data){
					if(!data) {
						selectTag(_this.state.inputTag);
					}
				}
			});
		},
		handleSelectTag: function(event) {
			...
			ele.parents('ul').siblings('input[type=text]').before('<span class="tag selected">'+WQ.trim(_this.state.inputTag)+'<span class="remove">x</span></span>');
			_this.state.tags.push(WQ.trim(_this.state.inputTag));
			_this.setState({
				tags: _this.state.tags,
				inputTag: ''
					...
					React.createElement("div", {className: "edit-article"}, 
						React.createElement("form", null, 
				            ...
				            React.createElement("div", {className: "tags-list", onClick: _this.getFocus}, 
				            	...
				            	React.createElement("ul", null, tags, React.createElement("li", {style: _this.state.cacheTags.length>4 ? {display:'none'} : {display:'block'}, onClick: _this.dialogShow}, React.createElement("a", {href: "javascript:void(0)"}, "创建标签 ", React.createElement("strong", null, _this.state.inputTag))))
				            ), 
