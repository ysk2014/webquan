	...
	return React.createClass({
		...
		dealTags: function(event) {
			...
			if((event.which==13 || event.which==32) && WQ.trim(event.target.value)!='') {
				var len = ele.siblings('.tag.selected').length
				if(len>2) {
					ele.val('');
					return;
				}
				ele.before('<span class="tag selected">'+event.target.value+'<span class="remove">x</span></span>');
				_this.state.tags.push(event.target.value);
				_this.setState({
					tags: _this.state.tags.push(event.target.value)
				});
				ele.val('');
