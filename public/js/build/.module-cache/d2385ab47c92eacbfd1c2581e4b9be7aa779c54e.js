define([
	...
	var mixin = {
		...
		handleCare: function(event) {
			...
			var index = $(event.target).data('key');
			var myCare = $(event.target).data('care');
			var cid = $(event.target).parents('li').data('cid');
			var uid = WQ.cookie.get('id');
				...
				CloumnModel.addCare(dataObj,function(success,data) {
					if(success) {
						if(!data.error) {
							if(nav==0) {
								if(navChild==0) {
									_this.state.cloumns['view'][index]['myCare'] = true;
								} else if(navChild==1) {
									_this.state.cloumns['update_time'][index]['myCare'] = true;
								} else if(navChild==2) {
									_this.state.cloumns['care'][index]['myCare'] = true;
								} else if(navChild==3) {
									_this.state.cloumns['addtime'][index]['myCare'] = true;
								}
							} else if(nav==1) {
								_this.state.cloumns['myCare'][index]['myCare'] = true;
							} else if(nav==2) {
								_this.state.cloumns['me'][index]['myCare'] = true;
							}
						} else {
							Tooltip(data.msg);
						}
		...
		handleOver: function(event) {
			var span = $(event.target).find('span:eq(0)');
			if($(event.target).find('span:eq(0)').html() =='正在') {
				$(event.target).find('span:eq(0)').html('取消');
			}
		},
		handleOut: function(event) {
			if($(event.target).find('span:eq(0)').html() =='取消') {
				$(event.target).find('span:eq(0)').html('正在');
			}
		},
	}
					...
					React.createElement("li", {key: d.id, "data-cid": d.id, "data-key": i, className: "cloumn"}, 
						React.createElement("a", {href: "/cloumn/"+d.id}, 
							...
							React.createElement("p", null, 
								...
								React.createElement("a", {className: "btn-success", "data-care": d.myCare ? d.myCare : false, onClick: _this.handleCare, onMouseEnter: _this.handleOver, onMouseLeave: _this.handleOut}, d.myCare ? '正在' : '添加', "关注")
							)
						)
					)
				);
