	...
	var mixin = {
		...
		hamdleTabChange: function(event) {
			...
			if(!_this.state.list[way]) {
				if(way=='care') {
					_this.getAllArtsByUidCare(uid,0);
				} else {
		...
		// 获取专题文章
		getAllArtsByUidCare: function(uid,page) {
			var _this = this;
			ArticleModel.getAllArticleByCid({uid:cid,way:'addtime',page:page},function(success,data) {
				if(success) {
					if(!data.error) {
