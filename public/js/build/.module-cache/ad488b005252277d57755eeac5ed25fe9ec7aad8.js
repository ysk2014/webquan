		...
		init: function() {
			...
			if(aid>0) {
				ArticleModel.getArticleById(aid,function(success,data) {
					if(success) {
						if(!data.error) {
							if(_this.state.info.tags.indexOf('|')) {
								_this.state.info.tags = _this.state.info.tags.split('|');
							} else {
								_this.state.info.tags = _this.state.info.tags;
							}
							_this.state.info.tags = _this.state.info.tags.split('|');
							_this.setState({
								info: data.data,
	...
	return React.createClass({
		...
		render: function() {
			...
			var tagsSpan = _this.state.tags.length>0 ? _this.state.tags.map(function(tag,i) {
				return (React.createElement("span", {className: "tag selected"}, tag, React.createElement("span", {className: "remove"}, "x")));
			}) : null;
