	...
	return React.createClass({
		...
		dealTags: function(event) {
			if((event.which==13 || event.which==32) && WQ.trim(event.target.value)!='') {
				var len = $(event.target).siblings('.tag.selected').length
				$(event.target).before('<span class="tag selected">'+event.target.value+'<span class="remove">x</span></span>');
				$(event.target).val('');
				$(event.target).siblings('.selected').find('.remove').on('click',function() {
					$(this).parent().remove();
				});
			} else if(event.which==8 && WQ.trim(event.target.value)=='') {
				$(event.target).prev('.tag').remove();
			}
		},
		render: function() {
			var _this = this;
