    ...
    'home/common/userDropMenu',
		...
		var Head = React.createClass({displayName: "Head",
			...
			componentDidUpdate: function() {
				var _this = this;
				// var id = WQ.cookie.get('id');
				// var str = '<form enctype="multipart/form-data" action="/user/updateLogo" method="post" target="uploadIframe" >'+
				...
				// 			'<a class="submit-button">上传头像</a>'+
				// 		'</form>';
				// $('.head').append(str);
				$('.head .uploadForm').attr('enctype','multipart/form-data');
				// $('.head').on('click','.submit-button',function() {
				// 	$(this).siblings('input[type=file]').trigger("click");
				// });
				// $('.head').on('change','#file',function() {
				// 	if($(this).val() === '') {
				// 		Tooltip('头像不能为空');
				// 	}
				// 	$(this).siblings('input[type=submit]').trigger('click');
 			// 	});
 			// 	$('.head').on('click','input[type=submit]',_this.handleUpload);
			},
			render: function(){
				var _this = this;
				return(
					React.createElement("div", {className: "head"}, 
						React.createElement("div", {className: "user", style: {background:'url("'+_this.state.logoDir+'") no-repeat center'}}), 
							React.createElement("form", {className: "", enctype: "multipart/form-data", action: "/user/updateLogo", method: "post", target: "uploadIframe"}, 
														
								React.createElement("iframe", {name: "uploadIframe", id: "uploadIframe", style: "display:none"}), 

								React.createElement("input", {type: "text", name: "id", style: "display:none", value: WQ.cookie.get('id'), onChange: _this.handleFileChange}), 
								React.createElement("input", {type: "file", name: "file", id: "file", style: "display:none"}), 
