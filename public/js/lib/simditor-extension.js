...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD. Register as an anonymous module unless amdModuleId is set
    define('simditor-preview', ["jquery","simditor"], function (a0,b1) {
      return (root['SimditorPreview'] = factory(a0,b1));
    });
  } else if (typeof exports === 'object') {
    // Node. Does not work with strict CommonJS, but
    // only CommonJS-like environments that support module.exports,
    ...
    root['SimditorPreview'] = factory(jQuery,Simditor);
  }
}(this, function ($, Simditor) {
  var SimditorPreview,
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    ...
    SimditorPreview = (function(superClass) {
      extend(SimditorPreview, superClass);

      function SimditorPreview() {
        return SimditorPreview.__super__.constructor.apply(this, arguments);
      }

      SimditorPreview.prototype.name = 'preview';

      SimditorPreview.prototype.needFocus = false;

      SimditorPreview.prototype.iconClassOf = function() {
        return 'fa fa-eye';
      };

      SimditorPreview.prototype._init = function() {
        SimditorPreview.__super__._init.call(this);
        this.wrapper.on('click', 'a.toolbar-item-preview', (function(_this) {
          return function(e) {
            e.preventDefault();
            var val=_this.editor.getValue();
            var tpl = '<div class="simditor-preview"><div class="simditor-preview-wrapper">'+
                        '<a href="javascript:;" class="fa fa-close simditor-preview-close-btn"></a>'+
                        '<div class="simditor-body">'+val+'</div>'+
                      '</div></div>';
            $('body .simditor').append(tpl).append('<div class="simditor-preview-mask"></div>');
            $('body').css('overflow','hidden');
            $('body .simditor').on('click','.simditor-preview-mask,.simditor-preview-close-btn', function() {
              $('body .simditor .simditor-preview').remove();
              $('body .simditor .simditor-preview-mask').remove();
      ...
      return SimditorPreview;
    })(Simditor.Button);

    Simditor.Toolbar.addButton(SimditorPreview);
